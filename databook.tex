...
When you run Git Bash, you will see some welcome and help messages, followed by a \newterm{prompt} such as:\\
...
The prompt gives us some useful information.  It tells us who is logged in, and what directory/folder you're currently working in (the \verb|~| is an abbreviation for the user's home directory).  The prompt is customizable, and will probably appear differently to users on Mac and Linux systems.  From here on out, in my examples I will simply use \verb|$| to represent the command prompt.  To execute a command, type it at the prompt and press Enter.  Some of the commands you can try are \code{ls} (list files in the directory) and \code{cd Documents} (change to the "My Documents" folder on Windows).

Now, let's see if your computer is set up to use Python.  At the prompt (represented here by the \code{\$}), type:

\mint{shell}|$ python|

If you got an error, first: did you type the \verb|$|?  You weren't supposed to type that!  The \verb|$| is just my stand-in for the command prompt which will be different on every student's computer.  You should have typed the word \verb|python| only, and then hit Enter.

If that's not the problem, then you might have gotten an error saying \verb|"python not found"| or something similar.  To fix this, you need to update your operating system's \verb|PATH| variable.  This is a setting that tells the computer where, when someone enters a command, to look for the program the user indicated.  For a Windows user, Python 3.4 will be installed in the directory \verb|C:\Python34\| and some other necessary scripts will be in \verb|C:\Python34\Scripts\|.  You'll need to find your existing \verb|PATH| and add those two directories at the end of it.  Instructions for Windows users can be found here: \url{http://www.computerhope.com/issues/ch000549.htm}.  On a Macintosh I'm not quite sure where Python will be installed by default, but some instructions for setting the \verb|PATH| are found here: \url{http://www.tech-recipes.com/rx/2621/os_x_change_path_environment_variable/}.  You'll want to close your terminal and open a new one after changing the \verb|PATH|.

If everything goes well, the next time you enter the \verb|python| command, the Python interpreter will open.  It will tell you the version of python you are running, and give you a new prompt, probably \verb|>>>|.  At this prompt, you can enter lines of Python code, but you cannot enter operating system commands.  Try entering the following at the prompt.  (Don't enter the \verb|>>>|!)
...
At any time you can type \code{quit()} to exit the Python interpreter and return to the operating system prompt.
...
Save the file as \code{hello.py}.  The "\verb|.py|" extension to the filename tells Python and other programs that this is a Python program.  Be careful that your text editor doesn't automatically add a "\verb|.txt|" extension instead.  To run your new program, navigate to the folder/directory where you saved the file, and type the following:

\mint{shell}|$ python hello.py|

The program will run, print some sentences to the terminal, and ask you to press Enter.  Once you do, you'll be returned to the command prompt (\code{\$}).  You are now set up to write any Python programs you want, and run them.  If this doesn't work, you may still need practice using the command prompt, or may need help to set your \verb|PATH| correctly.  Also, make sure you're at the operating system command line (\verb|$|) and not inside the Python interpreter (\verb|>>>|) when you enter \verb|python hello.py|.

\subsection*{Writing some data to a file}

In our next program, we're going to write some data to a file in the \newterm{CSV} (comma-separated values) format.  This is one way of organizing some data to share it with others.  Its limitations will be considered in the next chapter.  Within the code, I will use \emph{comments}.  These lines, beginning with \verb|#|, are ignored by Python, so we use them to provide explanations for other people who may have to maintain or use our code.
...
Were you able to follow everything that was going on in the code?  Python is pretty close to English, so sometimes you can just guess.  If you are struggling, you may want to search the Internet for help with the keywords \code{while} and \code{if}.  Don't be shy about asking for help!
...
Imagine, for example, that your application is a Twitter-like microblog.  The API should have a ULR endpoint for each \newterm{entity} in the microblog's universe.  These "entities" are its nouns; some are individual objects: users, messages, hashtags, etc., and others are \newterm{collections}, such as the set of all messages posted by a certain user.  Your API should provide a way to access both objects and collections, and it should use the four main HTTP methods in the correct way, as its verbs.

An initial sketch of the API documentation might include the following:\\

\code{GET /users}: List all users of the system (usernames and real names)

\code{POST /users}: Create a new user

\code{GET /users/\{username\}}: Retrieve all data about the specified user

\code{GET /users/\{username\}/posts}: Retrieve all posts by this user

\code{DELETE /users/\{username\}}: Delete the specified user account\\

And similar methods would be documented for URL endpoints for messages, hashtags, and other data in the application.  After the API endpoints, which correspond to resources, your API may provide a number of optional parameters to filter the response.  For example, instead of all messages, your user may only want the last 10, or only want messages posted during the last month.  In a URL, these parameters follow a \code{?} and are separated by \code{\&}.  For example:\\

\code{GET /messages?limit=10}: Retrieve the last 10 messages.

\code{GET /messages?limit=10\&offset=10}: Retrieve 10 messages \emph{after} the first 10 (in other words, retrieve "page 2" of messages).

\code{GET /messages?month=January\&year=2015}: Retrieve all messages from January 2015.

Some other good practices in setting up your API are to choose a good URL root.  You could use something like \code{http://www.mysite.com/api/*} (the \code{*} is where your consumers put the  specified URL endpoints), or you could use \code{http://api.mysite.com/*}.  You may also want to include a version number, so that if you change the API, consumers can still use the old version.  For example: \code{http://api.mysite.com/v1/*}.
