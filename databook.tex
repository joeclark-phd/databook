...
\todo{How to install Python and get it running (i.e. set your PATH)}

\todo{Provide some basic code to ask the user for inputs -- name, address, etc.}

\todo{Introduce some string functions and show how to append the data to a file.}

\todo{Introduce a second script that reads the file and prints the data.}

\todo{Propose some challenges: How to include metadata?  How to encode the file without using certain functions?  How to allow for optional data fields?  How to structure the data for easy searching (for example, alphabetizing or creating an index?  How to sum or average?}
...
\todo{No, not HTML... it's HTTP!  Describe how web servers work, how browsers and servers talk to one another.}

\section{Communicating in Data}
\todo{Talk about the problems of the Internet: statelessness, unreliability, etc.  Build up to REST and concepts like idempotence.}
...
\begin{itemize}
    ...
    \item Jurney, R. (2014).  Agile Data Science.  O'Reilly.
    \item Mahemoff, M. (2006).  Ajax Design Patterns:  Chapter 9: Web Services. (pp.161-207). O'Reilly.
    \item \todo{Something good on API design for web services?}
\end{itemize}
