...
\mint{shell}|$ cd Desktop/flinkedin|
...
The best way I've found to get a MongoDB database running in the cloud is to use Mongolab (\url{http://mongolab.com}).  

Register for an account. You will have to verify your e-mail address. After signing in you will see that you have created an "account" and a "user".  The account could be for a company or project and there could be multiple users in one account.

Click "Create New" to create a new database.  Select a single-node deployment on Amazon Web Services of the "Sandbox" plan, which is free of charge.  At the time of writing, this was only available in Amazon's US East region.  It will give you 0.5GB of storage on a shared node---enough to experiment with but not enough power for a commercial application.  Give the database a name; mine is called |cis355sandbox|.  Click "Create new MongoDB deployment" when you're ready.

Click on the database name, and you'll get some information including the instructions to connect to the database.  The first thing you'll need to do is to create a database user.\footnote{Your Mongolab user account gives you access to the account, including the right to create and delete databases; the database user account gives you rights within the database.  Neither of these is the same as your Amazon Web Services account.  I recommend using a password manager to keep track of all these details.}  If you successfully installed the MongoDB client (|mongo|) on your personal computer, and created a database user, you can connect using the command line:

\mint{shell}|$ mongo <connection string> -u <dbuser> -p <dbpassword>|

If you got into MongoDB, great!  Try a couple of commands to make sure:


\begin{minted}{shell}
> db.testcollection.insert({ "go" : "sun devils" })
WriteResult({ "nInserted" : 1 })
> db.testcollection.find()
{ "_id" : ObjectID("..."), "go" : "sun devils" }
> exit
\end{minted}

By leveraging Mongolab's cloud service, we have avoided the painstaking work of installing and configuring MongoDB on our own personal computers.  We can continue to use this free server for our remaining exercises.

\subsection*{Setting up a database backend}

Recall that our app "flinkedin" used hard-coded data so that its "database" (if it can be called that) resets every time the server is restarted.  We'll now adapt the code so that the user profile data is stored in a MongoDB database for persistence over time and across versions.

You might want to make a copy of the |application.py| file from "flinkedin" that you developed doing the tutorials from Chapter \ref{ch:apis} and Chapter \ref{ch:cloud}.  Make a new folder for the copy, and don't bother copying the virtual environment or other files, because we're going to be changing some things.

The first thing we're going to do is create a new virtual environment, because this applicaiton will use another Python package that wasn't needed in the previous tutorial.  The appropriately-named package |pymongo| enables Python to connect to MongoDB to send and receive data. So, navigating into the top-level folder for this application, first create a virtual environment:

\mint{shell}|$ virtualenv venv|

And then use |pip| to install Flask and |pymongo| in the virtual environment:

\mint{shell}|$ venv/Scripts/pip install flask pymongo|

Since we're going to upload this to Amazon, we'll need to update |requirements.txt| with this new dependency.  You can do so with this one-liner:

\mint{shell}|$ venv/Scripts/pip freeze > requirements.txt|

Now we'll create a script to set up our database with some initial data.  This script will only run once, so it's not going to be part of the web application we upload to Amazon, but it's a good way to demonstrate how Python connects to MongoDB.

The first thing we do is import the function |MongoClient| from the |pymongo| package (line 1 of the example below).  In line 7, we create a connection called |client| by calling the |MongoClient| function with the URL of the database.  Remember that your database connection details are different from mine and can be found on the Mongolab website.  Remembering that a MongoDB server can contain multiple databases (really just named groups of collections), line 10 of the example creates the |db| variable as a direct link to the default database on our MongoDB server.
...
\begin{minted}[firstnumber=15]{py}
john =  { "_id" : 123,
          "fname" : "John",
          "lname" : "McClane",
          "jobs" : [ 
...
\begin{minted}[firstnumber=23]{py}
...
\begin{minted}[firstnumber=30]{py}
...
\begin{minted}[firstnumber=42]{py}
...
\begin{minted}[firstnumber=49]{py}
