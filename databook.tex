...
In our next program, we're going to write some data to a file in the \term{CSV} (comma-separated values) format.  This is one way of organizing some data to share it with others.  Its limitations will be considered in the next chapter.  Within the code, I will use \emph{comments}.  These lines, beginning with \verb|#|, are ignored by Python, so we use them to provide explanations for other people who may have to maintain or use our code.
